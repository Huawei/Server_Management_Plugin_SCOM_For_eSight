...
namespace Huawei.SCOM.ESightPlugin.RESTeSightLib
{
    ...
    public class ESightEngine
    {
        ...
        private void RefreshPwds()
        {
            try
            {
                HWLogger.UI.Info("Refresh eSightpwd with encryption...");
                        ...
                        using (var mutex = new Mutex(false, "huawei.SCOM.ESightPlugin.engine"))
                        {
                            if (mutex.WaitOne(TimeSpan.FromSeconds(60), false))
                            {
                                ...
                                if (!EncryptUtil.IsCompatibleVersion())
                                {
                                    ...
                                    HWLogger.UI.Info($"oldMainKey:{oldMainKey}");
                                ...
                                HWLogger.UI.Info($"Start Refresh eSightpwd !eSight Count:{hostlist.Count}");
                                foreach (var eSightHost in hostlist)
                                {
                                    try
                                    {
                                        ...
                                        //HWLogger.DEFAULT.Info($"6:{eSightHost.HostIP}");
                                        //this.eSightSessions[eSightHost.HostIP.ToUpper()] = iEsSession;

                                        ESightDal.Instance.UpdateESightPwd(eSightHost.HostIP, enPwd);
                                        HWLogger.UI.Info($"Refresh eSightpwd :{eSightHost.HostIP}!");
                                    }
                                    catch (Exception ex)
                                    {
                                        HWLogger.UI.Info($"Refresh eSightpwd  Error.eSight{eSightHost.HostIP}", ex);
                                    }
                                }
                                // 更新完密码，刷新缓存
                                this.InitEsSessions();
                ...
                HWLogger.UI.Info("Refresh eSightpwd with encryption successful!");
            }
            catch (Exception ex)
            {
                HWLogger.UI.Error("Refresh eSightpwd  Error!", ex);
            }
        }
